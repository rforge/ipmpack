...
sampleIPM<- function(
  ...
  if(!is.null(growObjList)){
    PmatrixList <- .makeListPmatrix(growObjList=growObjList, survObjList=survObjList, nBigMatrix=nBigMatrix, minSize=minSize, maxSize=maxSize, cov=covariates, envMat=envMat,discreteTrans=discTransList, integrateType=integrateType, correction=correction) 
