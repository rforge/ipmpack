...
# Wrapper function to fit Bayes, and make pics
# of life expect, passage time, with CIs
#
# Parameters - dataf - the dataframe
#            - covsurv - should a discrete covariate be fit to survival
#            - covgrow - should a discrete covariate be fit to growth
#            - target.size - the target size for passage time (set to 0.2 of the mean if not provided)
#            - nitt - the number of iterations used in the MCMC fit
#            - nsamp - the number of IPMs built by sub-sampling the MCMC fit posteriors
#            - n.big.matrix - the number of bins
#            - minsize - the minimum size (defaults to numeric with length zero - then calculated)
#            - maxsize - the maximum size (ditto)
#            - do.fec - do you want to fit fertility
#            - covfec - should fertility preprod function fit a discrete covariate (if it is fitted at all...)
#            - covseeds - should fertility nseeds function fit a discrete covariate (if it is fitted at all...)
#            - env.mat - matrix of env transitions (only if cov=TRUE)
#            - is.log - is data on a log scale - for plotting conversions
#            - yrs.census - interval between censuses (to multiply LE and passage time)
#            - number of size bins for which size to age desired
#
# Returns - list including a matrix of the posterior of life expectancy and passage time,
#           both with size as columsn, samples as rows, lambda and stable size dist
#          The last two are NAs if fertility not fit
#
#
run.Simple.Model.Bayes <- function(dataf,covsurv=FALSE,covgrow=FALSE,
                                   target.size=c(),
                                   nitt=50000,nsamp=100,
                                   n.big.matrix=500,minsize=c(),maxsize=c(),
                                   do.fec=FALSE,covfec=FALSE,covseeds=FALSE,
                                   env.mat=NULL, is.log=TRUE, yrs.census=1,
                                   n.size.to.age=0,size.start=log(10)) {
   ...
   if (is.log) {
    ...
    #remove NAs, since makes MCMCglmm unhappy
    dataf1 <- dataf[!is.na(dataf$size) & !is.na(dataf$surv),]
    dataf2 <- dataf[!is.na(dataf$size) & !is.na(dataf$sizenext),]
    dataf3 <- dataf[!is.na(dataf$size) & !is.na(dataf$fec),]
    
    #run Bayes
    sv1 <- makePostSurvivalObjs(dataf=dataf1, covsurv=covsurv,nitt=nitt)
    sv1 <- sample(sv1,size=nsamp,replace=FALSE)
    gr1 <- makePostGrowthObjs(dataf=dataf2, covgrow= covgrow, nitt=nitt)
    gr1 <- sample(gr1,size=nsamp,replace=FALSE)

    if (do.fec) {
        fv1 <- makePostFecObjs(dataf=dataf3, covfec= covfec,covseeds=covseeds,nitt=nitt)
        fv1 <- sample(fv1,size=nsamp,replace=FALSE)
    } else { fv1 <- NULL } 
    ...
    # set up target size if not defined
    if (length(target.size)==0)  target.size <- 0.7*(minsize+maxsize)
    
    #run code that builds IPMs, and returns key output
    res <- getIPMoutputDirect(survObjList=sv1,growObjList=gr1,target.size=target.size,
                              n.big.matrix=n.big.matrix,minsize=minsize,maxsize=maxsize,
                              cov=covsurv,fecObjList=fv1, env.mat=env.mat,n.size.to.age=n.size.to.age,
                              size.start=size.start)

   
   res$LE <- yrs.census*res$LE
   res$ptime <- yrs.census*res$ptime
   res$resAge <- yrs.census*res$resAge
      
    #picture
    par(mfrow=c(2,2),bty="l")
    nmesh <- length(res$meshpoints)
    meshpoints <- res$meshpoints
    CI <- apply(res$LE,2,quantile,c(0.025,0.975))
    
    plot(conv(meshpoints),colMeans(res$LE), type="n",xlab="Size", ylab="Life expectancy",
         ylim=range(quantile(res$LE,c(0.025,0.975))), xlim=conv(range(dataf$size)),log=axes)
     
    polygon(conv(meshpoints[c(1:nmesh,nmesh:1)]),
            c(CI[1,1:nmesh],CI[2,nmesh:1]),col="grey",border="white")
    points(conv(meshpoints),colMeans(res$LE),type="l",lty=3)

    CI <- apply(res$ptime,2,quantile,c(0.025,0.975))

    plot(conv(meshpoints),colMeans(res$ptime),
         type="n",xlab="Size", ylab="Passage time", xlim=conv(c(min(dataf$size,na.rm=TRUE),0.9*target.size)),
         ylim=range(quantile(res$ptime[,meshpoints<0.9*target.size],c(0.025,0.975))),log=axes)

    polygon(conv(meshpoints[c(1:nmesh,nmesh:1)]),
            c(CI[1,1:nmesh],CI[2,nmesh:1]),col="grey",border="white")
    points(conv(meshpoints),colMeans(res$ptime),type="l",lty=3)
    ...
    if (do.fec) { 
        ...
        plot(conv(meshpoints),colMeans(res$stable.size), type="n",xlab="Size", ylab="Stable.size",
             ylim=range(quantile(res$stable.size,c(0.025,0.975))),log=axes)
        polygon(conv(meshpoints[c(1:nmesh,nmesh:1)]),
                c(CI[1,1:nmesh],CI[2,nmesh:1]),col="grey",border="white")
        points(conv(meshpoints),colMeans(res$stable.size),type="l",lty=3)
