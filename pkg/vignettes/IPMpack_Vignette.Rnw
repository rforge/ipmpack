...
<<>>= 
IPMlist <- makeListIPMs(dataf = dff, nBigMatrix = 50, minSize = -5, 
...
<<>>= 
r <- stochGrowthRateManyCov(covariate = covMatTest, nRunIn = 12*7, 
                            tMax = length(tVals), growthObj = gr1, 
                            survObj = sv1, fecObj = fv1, nBigMatrix = 50, 
...
<<>>= 
Fmatrix <- createIPMFmatrix(fecObj = fv1, nBigMatrix = 50, 
...
fv <- makePostFecObjs(dff, explanatoryVariables = "size+size2", fecConstants=0.01, 
                      burnin=50,nitt = 80, Transform = "log")
