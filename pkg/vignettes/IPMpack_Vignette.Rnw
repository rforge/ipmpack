...
This results from the fact that continuous distributions of the kernal are discretized when making the IPM matrix, and  Other approaches to this problem are in development (see {\tt ?diagnosticsTmatrix} for additional details).
