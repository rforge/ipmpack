...
matrix and extending the number of bins does not alter basic predictions from the IPM.
...
larger numbers are advisable (the default in MCMCglmm is 50,000). With output from this, the user can make a list containing multiple T matrices:
<<>>= 
