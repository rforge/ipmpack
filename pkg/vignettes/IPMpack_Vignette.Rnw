...
implementation of IPMpack to biologists with a wide range of quantitative skills.  This vignette is for IPMpack version $1.3$, and so we encourage users to contact the IPMpack team at \href{IPMpack@gmail.com}{IPMpack@gmail.com} with any feedback or mistakes they find.  We also host a blog at R-forge (http://ipmpack.r-forge.r-project.org/) that contains news of updates, new features, and announcements of papers and meetings relevant to IPMs.
...
plots these against each other for the three matrices in the first panel (current, extended range and increased bin number) using the same colours as in the first panel. Lines should fall along the (0,1) line shown in grey; if they do not, the argument {\tt correction="constant"} in {\tt createIPMTmatrix} may be of use. This ensures that the columns sum to the fitted survival by multiplying every column in the Integral Projection Model by the value that allows this. The third panel checks whether extending the size range included in the matrix and increasing the number of bins (by increasing {\tt nBigMatrix} and thereby having narrower bins) does not alter basic predictions from the IPM.

The six panels on the next plot show the discretized IPM (histograms)for the current IPM (top) and one with an increased number of bins (bottom)  and the
...
Other useful function for verifying that sufficient bins have been used include {\tt convergeLambda} and associated functions. 
...
slotNames(Tmatrix)
@ 
and finally, the user can plot the Tmatrix using {\tt persp} or {\tt
...
image(Tmatrix@meshpoints, Tmatrix@meshpoints, t(Tmatrix), ticktype = "detailed", 
		  xlab = "Size at t", ylab = "Size at t+1")
contour(Tmatrix@meshpoints, Tmatrix@meshpoints, t(Tmatrix), add=TRUE)
@
\end{center}
...
functions can be used to create a compound T matrix, using {\tt createCompoundTmatrix}: 
<<>>= 
...
image(1:nrow(Tmatrix), 1:ncol(Tmatrix), t(log(Tmatrix)), 
...
@ 
\end{center}
\caption{Compound T matrix IPM shown on a log scale: whiter colours indicate
