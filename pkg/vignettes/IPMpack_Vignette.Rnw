...
\title{IPMpack: an R package for demographic modeling with Integral Projection Models (v.1.2)}
...
implementation of IPMpack to biologists with a wide range of quantitative skills.  This vignette is for IPMpack version $1.3$, and so we encourage users to contact the IPMpack team at \href{IPMpack@gmail.com}{IPMpack@gmail.com} with any feedback or mistakes they find.  We also host a blog at R-forge \href{http://ipmpack.r-forge.r-project.org/}{IPMpack Web Site} that contains news of updates, new features, and announcements of papers and meetings relevant to IPMs.
...
structures can easily be  accommodated with IPMpack. The $F$ kernel (equation 4)
...
($c_1$, $c_2$, ...) can be included if there are no data for a stage.  Finally, the $F$ kernel definition includes a probability density function describing the size of offspring recruiting into the population, $f_d$:
\begin{equation}
...
This vignette will now walk through the steps of a basic IPM analysis.  We first
...
\item  {\tt fec1, ...}: as many columns as desired relating size to sexual reproduction. For example, this might be: 
  ...
  \item ...
  \end{itemize}
  The default construction for the analytical part of IPMpack is such that any
  columns for which the column label contains $"fec"$ will be included in the
  analysis of the reproductive part of the life cycle (kernel $F$)
  automatically. This default can be over-ridden so that specific columns are
  identified for IPMpack fecundity functions to use.
...
simples case, ``continuous'' or ``dead''. As above, this column is not essential
...
\code{install.packages("IPMpack")} and then load IPMpack into an R session
(\code{library(IPMpac)}) (see help files for problems with installation or loading). 
...
outliers). The objects \code{growObj} and \code{survObj} define changes in size
and survival as defined above. 

\begin{figure}
\begin{center}
<<label = fig:diag, fig = TRUE, echo = FALSE>>= 
diagnosticsTmatrix(Tmatrix, gr1, sv1, dff)
@
\end{center}
\caption{Diagnostic plots for the Tmatrix.}
\label{fig:diag}
\end{figure}

IPMpack includes a useful function \code{diagnosticsTmatrix} that provides a
series of plots indicative of whether bin choice and size range is adequate.
Applying this function as a preliminary step before obtaining demographic and evolutionary output from IPMs can help identify
basic problems in the fitting of vital rate functions or in the creation of the
IPM matrices before proceeding. Several common problems can be diagnosed with
the panelled figure produced by \code{diagnosticsTmatrix} (Figure
\ref{fig:diag}).
...
\tt{maxSize} arguments in \code{createIPMTmatrix}.  One common problem
...
this, \code{createIPMTmatrix} has an argument {\tt correction = "constant"}
which ensures that the columns sum to the fitted survival by adding a constant. 
The top-right panel checks whether extending the size range included in the
matrix is sufficient to include all dynamics derived from the vital rate models.
The three lower panels show whether or not the discretized IPM and the
theoretical density function.  If mismatched, increasing the tt{nBigMatrix}
argument may correct this discrepency.  

The {\tt createIPMTmatrix} function builds around methods defined so that it
will provide appropriate output whatever the survival and growth objects are
(e.g. error structure, covariates). The T matrix contains a matrix defining the
transitions, but also other useful slots, e.g., the meshpoints, covariates,
grid size.
The user can access this information by writing:
<<>>= 
slotNames(Tmatrix)
@ 
...
persp(Tmatrix@meshpoints, Tmatrix@meshpoints, Tmatrix, ticktype = "detailed", 
...
@ 
and the user can also plot these against {\tt Tmatrix@meshpoints} to examine how
life expectancy and passage vary as a function of size (Figure~\ref{fig:three} p.~\pageref{fig:three}).
\begin{figure}
\begin{center}
<<label = fig3, fig = TRUE, echo = FALSE>>= 
par(mfrow = c(1, 2), bty = "l")
plot(Tmatrix@meshpoints, LE, type = "l", xlab = "Size", 
     ylab = "Mean life expectancy", 
     xlim = range(dff$size, na.rm = TRUE), 
     ylim = range(LE[Tmatrix@meshpoints<max(dff$size, na.rm = TRUE)]))   
plot(Tmatrix@meshpoints, pTime, type = "l", 
     xlab = "Size at start", ylab = "Time to reach chosen size", 
     xlim = range(Tmatrix@meshpoints[Tmatrix@meshpoints<mean(dff$size, na.rm = TRUE)]), 
     ylim = range(pTime[Tmatrix@meshpoints<max(dff$size, na.rm = TRUE)]))
...
target size (i.e., here type: {\tt runSimpleModel(dff, chosenSize = 4)}) and runs this analysis to create figures for survival, growth, life expectancy and passage time as shown so far, assuming the simplest possible models of survival and growth (basic linear and logistic regressions, no covariates, etc).
...
offspringTypeRates=data.frame(NA)} and {\tt fecByDiscrete=data.frame(NA)}.
...
The function {\tt makeClonalObj} operates identically to {\tt makeFecObj} except that offspring are only considered for fitting the distribution of mean and standard deviation of offspring size if the column {\tt offspringNext} takes the values {\tt clonal}.

The user can combine the F matrix with (an identically built, i.e., same bin number, size limits and discrete classes) survival-growth transition T matrix to obtain a full Integral Projection Model, and its population growth rate $\lambda$, sensitivity, elasticity, etc. 
<<>>= 
IPM <- Tmatrix + Fmatrix
eigen(IPM)$value[1]
...
plot(Tmatrix@meshpoints, abs(Re(eigen(IPM)$vector[ , 1])), 
		 type = "l", xlab = "Size", 
     ylab = "Stable size structure")
image(Tmatrix@meshpoints, Tmatrix@meshpoints, log(sensitivity), 
		 xlab = "Size at t", 
      ylab = "Size at t+1", main = "Sensitivity")
image(Tmatrix@meshpoints, Tmatrix@meshpoints, log(elasticity), 
		 xlab = "Size at t", 
...
table(dff$stage)
@ 
Given this data structure, the user can make a fertiliy object that reflects the
...
Fmatrix <- createIPMFmatrix(fecObj = fv1, nBigMatrix = 5, 
                            ...
                            correction = "constant")
@
The user also needs a {\tt Tmatrix} that reflects the same structure. The
continuous part of the T matrix will be the standard structure:
<<>>= 
...
user can then construct the T matrix:
<<>>= 
...
print(Fmatrix)
@
and check for example that the slot {\tt namesDiscrete} is aligns them, and add
them together:
<<>>= 
...
#plot(Tmatrix@meshpoints, LEs[1, ], type = "l", xlab = "Size", 
#     ylab = "Mean life expectancy", ylim = c(0, 8), 
#     xlim = range(dff$size, na.rm = TRUE))   
#for (k in 1:Tmatrix@nEnvClass)  {
#	points(Tmatrix@meshpoints, LEs[k, ], type = "l", col = k)
#	}
plot(Tmatrix@meshpoints, pTimes[1:Tmatrix@nBigMatrix], 
     type  = "l", xlab = "Current size", 
     ylab = "Time to reach chosen size", 
     xlim = range(Tmatrix@meshpoints[1:14]), ylim = c(0, 8))
 for (i in 1:Tmatrix@nEnvClass) {
	points(Tmatrix@meshpoints, 
               pTimes[((i-1) * Tmatrix@nBigMatrix+1):(Tmatrix@nBigMatrix*i)], 
               type = "l", col = i)
  }
@
\end{center}
...
explanatory variables for the various linear and logistic
...
behavriors in that environment as if it were an IPM in a single environment.  
...
over $4$ years, with years as the time scale and built covariates that vary
...
is months.
Note that in this formula, it was assumed that density-dependence acts on
...
objects reflecting the parameter posteriors of the fitted linear and logistic
...
svlist <- makePostSurvivalObjs(dff, 
                               ...
                               burnin = 100, nitt = 400)
@ 
Note that the data must not contain NAs. This function currently only uses
default non-informative priors features. Note that there is a default thinning
in MCMCglmm, the engine used to derive the posteriors, that reduces simulations
by 10. So the number of samples from the posterior used in this simple example
{\tt nitt} is rather small (resulting in only [nitt - burnin] / 10 = 30), and
larger numbers are advisable (the default is 50,000). With output from this, the user can make a list containing multiple T matrices:
<<>>= 
...
matrix is provided. With this, the user can now obtain  posteriors for constant environment models. 
<<>>= 
...
Again, larger number of iterations, binsize, etc, are recommended. These results
...
plot(TmatrixList[[1]]@meshpoints, colMeans(res$LE), type = "n", 
     xlab = "Size", 
     ylab = "Life expectancy", 
     ylim = range(quantile(res$LE, c(0.025, 0.975), na.rm = TRUE)), 
     xlim = range(dff$size, na.rm = TRUE))
...
plot(TmatrixList[[1]]@meshpoints, colMeans(res$pTime), 
     type = "n", xlab = "Size", ylab = "Passage time", 
     xlim = range(dff$size[dff$size<5], na.rm = TRUE), 
     ylim = range(quantile(
     res$pTime[, TmatrixList[[1]]@meshpoints<max(dff$size, na.rm = TRUE)], 
...
polygon(c(TmatrixList[[1]]@meshpoints)[c(1:nMesh, nMesh:1)], 
        c(CI[1, 1:nMesh], CI[2, nMesh:1]), col = "grey", border = "white")
points(TmatrixList[[1]]@meshpoints, colMeans(res$stableStage, na.rm = TRUE), 
       type = "l", lty = 3)
...
function and the R function \code{optim}.
<<>>= 
