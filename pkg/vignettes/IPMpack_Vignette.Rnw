...
% switch this out before submission
<<>>= 
#library(IPMpack)
source("/Applications/eclipse/IPMpack/pkg/R/IPMpack-Impl.r")
source("/Applications/eclipse/IPMpack/pkg/R/IPMpack-Base.r")
source("/Applications/eclipse/IPMpack/pkg/R/IPMpack-Util.r")
@ 
...
<<>>= 
Tmatrix <- createIPMTmatrix(nBigMatrix = 100, 
...
fv1 <- makeFecObj(dff, explanatoryVariables = "size", 	
                  Family = "gaussian", 
                  Transform = "log")
Fmatrix <- createIPMFmatrix(nBigMatrix = 100, minSize = -5,
                            maxSize = 35, 
                            fecObj = fv1, 
                            correction = "constant")
@ 
...
<<>>= 
Tmatrix <- createCompoundTmatrix(nBigMatrix = 100, minSize = -5, 
                                 maxSize = 35, 
...
<<>>= 
IPMlist <- makeListIPMs(dataf = dff, nBigMatrix = 100, minSize = -5, 
...
number of time steps to run, and should be large enough that increasing it does not substantially change the result. 
...
fv1 <- makeFecObj(dataf = dff, fecConstants = 1.8, 
...
<<>>= 
r <- stochGrowthRateManyCov(covariate = covMatTest, nRunIn = 12*10, 
                            tMax = length(tVals), growthObj = gr1, 
                            survObj = sv1, fecObj = fv1, nBigMatrix = 100, 
                            minSize = 2*min(dff$size, na.rm = TRUE), 
                            maxSize = 1.5*max(dff$size, na.rm = TRUE), 
                            nMicrosites = 100, correction = "constant")
print(r)
@ 
Setting nRunIn = $12*10$ in this example is equivalent to discarding the first $10$ years (likely to contain transients) since the chosen time step is months. Note that in this formula, it was assumed that density-dependence acts on seedling establishment, and that $100$ microsites are available for seedling establishment in every time step. Setting nMicrosites = $0$ allows for calculations without density-dependence, and nMicrosites can also be a vector, if the number of microsites fluctuates through time. It may also be interesting to have a glance at what has been happening to the population structure over this time-course:
<<>>= 
st <- trackPopStructManyCov(covariate = covMatTest, 
                            nRunIn = 12*10, tMax = length(tVals), 
                            growthObj = gr1, 
                            ...
                            nMicrosites = 100, correction = "constant")
@ 
IPMpack contains a dedicated function to depict this, {\tt plotResultsStochStruct}, Figure~\ref{fig:six} (p.~\pageref{fig:six}).
@ 
\begin{figure}
\begin{center}
<<label = fig6, fig = TRUE, echo = FALSE>>= 
plotResultsStochStruct(tVals = tVals, st = st, covTest = covTest, log = "y")
@
\end{center}
\caption{Population structure through time}
\label{fig:six}
\end{figure}
...
<<>>= 
fv <- makePostFecObjs(dff, explanatoryVariables = "size+size2", fecConstants=0.01, 
                      burnin=100,nitt = 500, Transform = "log")
