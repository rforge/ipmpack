...
slotNames(Pmatrix)
@ 
and finally, the user can plot the Pmatrix using {\tt persp} or {\tt
...
image(Pmatrix@meshpoints, Pmatrix@meshpoints, t(Pmatrix), ticktype = "detailed", 
...
nMesh <- PmatrixList[[1]]@nBigMatrix
CI <- apply(res$LE, 2, quantile, c(0.025, 0.975))

plot(PmatrixList[[1]]@meshpoints, colMeans(res$LE), type = "n", 
     xlab = "Size", 
     ylab = "Life expectancy", 
     ylim = range(quantile(res$LE, c(0.025, 0.975), na.rm = TRUE)), 
     xlim = range(dff$size, na.rm = TRUE))
     
polygon(c(PmatrixList[[1]]@meshpoints)[c(1:nMesh, nMesh:1)], 
        c(CI[1, 1:nMesh], CI[2, nMesh:1]), col = "grey", border = "white")
points(PmatrixList[[1]]@meshpoints, colMeans(res$LE), type = "l", lty = 3)

CI <- apply(res$pTime, 2, quantile, c(0.025, 0.975))

plot(PmatrixList[[1]]@meshpoints, colMeans(res$pTime), 
     type = "n", xlab = "Size", ylab = "Passage time", 
     xlim = range(dff$size[dff$size<5], na.rm = TRUE), 
     ylim = range(quantile(
     res$pTime[, PmatrixList[[1]]@meshpoints<max(dff$size, na.rm = TRUE)], 
				 c(0.025, 0.975), na.rm = TRUE)))

polygon(c(PmatrixList[[1]]@meshpoints)[c(1:nMesh, nMesh:1)], 
        c(CI[1, 1:nMesh], CI[2, nMesh:1]), col = "grey", border = "white")
points(PmatrixList[[1]]@meshpoints, colMeans(res$pTime), type = "l", lty = 3)
...
plot(PmatrixList[[1]]@meshpoints, colMeans(res$stableStage), type = "n", 
		 xlab = "Size", ylab = "Stable.size", 
     ylim = range(quantile(res$stableStage, c(0.025, 0.975), na.rm = TRUE)))
polygon(c(PmatrixList[[1]]@meshpoints)[c(1:nMesh, nMesh:1)], 
        c(CI[1, 1:nMesh], CI[2, nMesh:1]), col = "grey", border = "white")
points(PmatrixList[[1]]@meshpoints, colMeans(res$stableStage, na.rm = TRUE), 
       type = "l", lty = 3)
